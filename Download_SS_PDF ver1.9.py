...
# 2024年8月26日 15:13:44 ver1.9 request访问被ban了，换成了httpx，并使用fake_useragent来自动生成随机伪装头；pypdf包大版本升级，语法变化很大，重写了一下。
# 2024年11月23日 20:28:00 ver1.9.1  fake_useragent没法打包进exe里面，舍弃了

import time,os,shutil,img2pdf,sys,re,cv2,glob
...
# }
my_headers= {"useragent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36 Edg/121.0.0.0"}
...
def GetData():
    ...
    # ua=fake_useragent.UserAgent()
    # my_headers = {'User-Agent':ua.random}
    respond=httpx.get(url=url,headers=my_headers,timeout=10)
    resp=respond.text.replace('\'','"')
    ...
    haveatry1=re.search(r'"bookinfo".*>(.*)<',resp)
    haveatry2=re.search(r'jpgPath: [\'"](.*)[\'"],',resp)
    while not haveatry1 or not haveatry2:
        # my_headers = {'User-Agent':ua.random} # 换个header再试
        respond=httpx.get(url=url,headers=my_headers,timeout=10)
        resp=respond.text.replace('\'','"')
    ...
    bookname=haveatry1.group(1)
    jpgpath=haveatry2.group(1)
    ...
    xml_r=httpx.get(url=xmlurl,headers=my_headers,timeout=20)
...
def DownloadCore(img_url,name):
    img=httpx.get(url=img_url,headers=my_headers,follow_redirects=True,timeout=30)
    result_bad=0
...
def AddContents(pdf_file,type_dict,contents_xml):
    ...
    sys.stdout.flush()
    time.sleep(0.01)
    # pdf_in=PdfReader(fileobj=pdf_file)
    pdf_out=PdfWriter()
    pdf_out.append(pdf_file)
    # for page in range(len(pdf_in.pages)):
    #     pdf_out.addPage(pdf_in.getPage(page))
    
    parent_id=[0, 0, 0, 0]
    parent_id[0]=pdf_out.add_outline_item("封面",0,parent=None)
    parent_id[0]=pdf_out.add_outline_item("版权页",0+type_dict['bok'][2]+type_dict['cov'][2],parent=None)
    parent_id[0]=pdf_out.add_outline_item("前言",0+type_dict['bok'][2]+type_dict['cov'][2]+type_dict['leg'][2],parent=None)
    parent_id[0]=pdf_out.add_outline_item("目录",0+type_dict['bok'][2]+type_dict['cov'][2]+type_dict['leg'][2]+type_dict['fow'][2],parent=None)
    ...
    for match in re.finditer(r'id=.*?InsertPageNumber',contents_xml):   #提取一条目录数据
        ...
        for level in list(range(4)):
            if index.count('-')==level: #数index字符串里的横杠来判断层级，不太优雅但works
                textcontents.append("\t"*level+caption+"\t"+str(pagenumber)+"\n")
                # print('\t'*level+caption+'\t'+pagenumber)
                if level==0 or parent_id[level-1]==0:
                    parent_id[level]=pdf_out.add_outline_item(caption,pagenumber,parent=None,color="#ff9632",bold=True)
                else:
                    parent_id[level]=pdf_out.add_outline_item(caption,pagenumber,parent=parent_id[level-1])
    
    with open("文本目录.txt",'w+',encoding="utf8") as f:
        f.writelines(textcontents)
...
def WritePDF(pdf_name,files_to_save,type_dict, contents_xml):
    ...
    with open("temp.pdf", "wb+") as pdf_temp:
        ...
        with open(pdf_name, "wb+") as pdf_file:
            pdf_out=AddContents("temp.pdf", type_dict, contents_xml)    #加目录
            pdf_out.write(pdf_file)
            pdf_out.close()                                     #输出 
    os.remove("temp.pdf")
...
def main():
    global Zoom,interval,max_retry,max_reretry
    print("Download_SS_PDF-ver1.9, by DertahSama, 2024.8.25")
