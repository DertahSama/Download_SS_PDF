...
def GetData():
    ...
    if "ssj.sslibrary.com" in url:
        print("本脚不能处理ssj.sslibrary.com打头的新式阅读器！请参考使用说明中给出的解决办法。")
        input("按任意键退出。")
    print("开始获取信息……")
...
def DownloadCore(img_url,index):
    ...
    img_pic.save("./RAW/%06d.png"%index,dpi=(dpi_set,dpi_set))
...
def Compress(type_dict):
    ...
    for index in range(1,all_pages+1):
        ...
        if index<=type_dict['cov']: #封面保持彩色
            ...
            imgdata=Image.open("./RAW/%06d.png"%index)
            dpi_set=imgdata.width/7.09  #固定页面宽度为18cm（7.09inch）的话，dpi应该就是img.width/7.09
            imgdata.save("./RAW/cmpresd/%06d.jpg"%index, optimize=True, quality=85, dpi=(dpi_set,dpi_set))
            ...
            dpi_set=imgdata.width/7.09  #固定页面宽度为18cm（7.09inch）的话，dpi应该就是img.width/7.09
            imgdata.save("./RAW/cmpresd/%06d.tiff"%index, format="TIFF", compression="group4",dpi=(dpi_set,dpi_set))
...
def main():
    ...
    print("Download_SS_PDF-ver1.7, by DertahSama, 2022.5.31")
    print("这是一个从超星图书馆（http://www.sslibrary.com ）下载PDF并且自动添加目录和压缩的python脚本，然后打包成了exe。")
    ...
    choose=input("（当前清晰度：zoom="+Zoom+"，当前下载间隔：%.1f s）"%interval+"要额外修改吗？(y修改，回车跳过)：")
    if choose=="y":
    ...
    choose=input("\n要压缩PDF到纯黑白吗？可大大减小文件体积(y确定，回车取消)：")
    if choose=="y":
        Compress(type_dict)
